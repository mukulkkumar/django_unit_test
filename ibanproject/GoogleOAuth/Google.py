...
class GoogleOAuth:
    ...
    def google_redirect(settings,request):
        ...
        try:
            ...
            request.session['google_state'] = state #Kind of CSRF token for Google OAuth
    ...
    def google_authenticate(request,settings):
        ...
        try:
            ...
            resp = requests.post(settings.ACCESS_TOKEN_URI, headers=headers, params=data)
    ...
    def get_google_profile(token_data,settings):
        ...
        try:
            ...
            resp = requests.get(settings.GOOGLE_PROFILE+"{accessToken}".format(accessToken=token_data['access_token']))
            return json.loads(resp.text)
        except Exception as e:
            return False